...
module Shipit
  class DeploysControllerTest < ActionController::TestCase
    ...
    test ":rollback button shows deploy and commit ids" do
      ...
      deploy_commit = @stack.commits.where(id: 4).first

      get :show, params: {stack_id: @stack, id: latest_deploy.id, format: 'html'}

      expected_result = "(Rollback of: <span class=\"short-sha\"><a class=\"number\" href=\"https://github.com/shopify/shipit-engine/compare/#{deploy_commit.sha}...#{rollback_commit.sha}\">#{deploy_commit.short_sha}...#{rollback_commit.short_sha}</a></span>)"
      assert_select 'span', {class: "deploy-status", html: expected_result}, "error"
    end
